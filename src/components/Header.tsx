...
import { Menu, X, User, LogIn, ChevronRight, Search, MessageCircle } from "lucide-react";
...
function AccountModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  function whatsappUrl() {
    ...
    const text = encodeURIComponent(
      `Olá! Gostaria de ajuda com meu pedido${code ? ` (código ${code.trim()})` : ""}.`
    );
        ...
        <div className="p-4 space-y-4">
          <div>
            ...
            <div className="mt-2 flex gap-2">
              <input
                ...
                placeholder="Digite o código (ex.: PS-20250916-ABCD)"
          ...
          <div className="pt-2 border-t">
            ...
            <p className="mt-1 text-xs text-zinc-500">
              Dica: você pode salvar o número do WhatsApp no <b>/checkout?admin=1</b>.
            </p>
          </div>
        </div>
      </div>
...
export default function Header() {
  ...
  // ativa menu mobile ao apertar ESC
  useEffect(() => {
    ...
    <>
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-zinc-200">
        <div className="mx-auto max-w-[1100px] px-4 py-3 flex items-center gap-3">
          ...
          <button
            onClick={() => setShowAccount(true)}
            className="lg:hidden inline-flex items-center gap-2 rounded-xl border border-zinc-300 bg-white px-3 py-2 text-sm font-medium text-zinc-800 hover:bg-zinc-50"
          >
            <LogIn className="h-4 w-4" />
            Entrar
          </button>
