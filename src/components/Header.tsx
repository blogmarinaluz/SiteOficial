...
function usePromoCountdown() {
  ...
  useEffect(() => {
    ...
    if (!deadline || Number.isNaN(deadline) || deadline < now) {
      deadline = now + 7 * 24 * 60 * 60 * 1000; // 7 dias
      try { localStorage.setItem(key, String(deadline)); } catch {}
    }
    const calc = () => {
      const diff = Math.max(0, deadline - Date.now());
      const s = Math.floor(diff / 1000);
      setRemain({ d: Math.floor(s/86400), h: Math.floor(s%86400/3600), m: Math.floor(s%3600/60), s: s%60 });
    };
    calc();
...
const samsungMenu = [
  ...
  "Galaxy A55","Galaxy A54","Galaxy A34","Galaxy A15",
];

function MegaFlyout({ label, href, items }: { label: string; href: string; items: string[] }) {
  return (
    <div className="relative group">
      <Link href={href} className="inline-flex items-center gap-1 text-sm text-neutral-700 hover:text-neutral-900">
        {label}<IconChevronDown className="h-4 w-4 opacity-70" />
      </Link>
      ...
      <div className="pointer-events-none absolute left-0 top-full z-50 opacity-0 transition-all duration-150 group-hover:pointer-events-auto group-hover:opacity-100">
        <div className="mt-2 w-[720px] rounded-2xl border border-neutral-200 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 p-4 shadow-[0_10px_30px_rgba(0,0,0,0.08)]">
          <div className="grid grid-cols-2 gap-1">
            {items.map((name) => {
              // >>> ENVIA SÓ O NOME DO MODELO em q (compatível com seu /buscar)
    ...
    <header className="w-full border-b border-neutral-100">
          ...
          <div className="grid grid-cols-1 gap-3 py-2 sm:grid-cols-[1fr_auto_1fr] sm:items-center">
            <div className="flex sm:justify-start justify-center">
              <div className="inline-flex items-center gap-2">
                ...
                <span className="text-sm sm:text-base font-semibold tracking-wide text-white">SEMANA DE PREÇOS BAIXOS</span>
              </div>
            </div>
      ...
      <div className="bg-neutral-50">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-2">
          <p className="text-xs sm:text-sm text-neutral-600">Especialista em celulares novos com garantia!</p>
        </div>
      </div>
      ...
      <div className="bg-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex items-center gap-4">
            <Link href="/" className="shrink-0">
              <span className="text-2xl font-extrabold select-none">
                <span className="text-neutral-900">pro</span><span className="text-emerald-600">Store</span>
              </span>
            </Link>
            ...
            <form action="/buscar" className="flex-1">
              <div className="flex items-stretch rounded-2xl border border-neutral-200 bg-white shadow-sm focus-within:ring-2 focus-within:ring-emerald-500/30 overflow-hidden">
                <input name="q" placeholder="Buscar por modelo, cor, armazenamento..." className="w-full px-4 py-2.5 text-sm outline-none placeholder:text-neutral-400" defaultValue="" />
                <button type="submit" className="inline-flex items-center gap-2 bg-emerald-600 px-4 py-2.5 text-white text-sm font-semibold hover:bg-emerald-700 transition-colors">
            ...
            <div className="ml-auto flex items-center gap-2 sm:gap-3">
              <Link href="/entrar" className="inline-flex items-center gap-2 rounded-xl border border-neutral-200 bg-white px-3 py-2 text-sm font-medium text-neutral-700 hover:bg-neutral-50">
                <IconUser className="h-5 w-5 text-emerald-600" /> Entrar
              </Link>
              <Link href="/carrinho" className="relative inline-flex items-center gap-2 rounded-xl border border-neutral-200 bg-white px-3 py-2 text-sm font-medium text-neutral-700 hover:bg-neutral-50">
                <IconCart className="h-5 w-5 text-emerald-600" /><span>Carrinho</span>
                {(typeof cartCount === "number" && cartCount > 0) && (
                  <span className="ml-1 inline-flex items-center justify-center rounded-full bg-emerald-600 px-1.5 py-0.5 text-[11px] font-bold leading-none text-white">{cartCount}</span>
                )}
              </Link>
            </div>
          </div>
          ...
          <nav className="mt-3 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-neutral-700">
            <MegaFlyout label="iPhone" href="/categoria/apple" items={iphoneMenu} />
            <MegaFlyout label="Samsung" href="/categoria/samsung" items={samsungMenu} />
            ...
            <div className="ml-auto flex items-center gap-3">
              <Link href="/analise-de-cadastro" className="inline-flex items-center rounded-full bg-amber-500 px-4 py-2 text-sm font-semibold text-white hover:bg-amber-600">Análise de Boleto</Link>
              <Link href="/checkout" className="text-sm font-medium text-neutral-700 hover:text-neutral-900">Checkout</Link>
            </div>
          </nav>
        </div>
      </div>
